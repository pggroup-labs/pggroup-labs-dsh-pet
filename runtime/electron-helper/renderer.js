...
class PetSprite {
  constructor(pet) {
    ...
    // 静态图宠物（media=image，如企鹅换皮）：按素材比例开窗，不被 16:9 压扁
    this.isImage = pet.media === 'image';
    this.aspect = typeof pet.aspect === 'number' && pet.aspect > 0 ? pet.aspect : this.isImage ? 1 : 9 / 16;
    this.height = this.size * this.aspect;
    this.halfW = this.size / 2;
    this.halfH = this.height / 2;
    this.bottomPad = (this.size * this.aspect * (S.CANVAS_H - S.FEET_Y)) / S.CANVAS_H;
    ...
    stage.style.setProperty('--pet-aspect', this.aspect + '');
    stage.style.transform = 'translateY(' + this.bottomPad + 'px)';
    this.stage = stage;
    // 静态图宠物：双 <img> 交叉淡入（同一固定 stage，切换不跳尺寸/位置）；否则双缓冲 video
    if (this.isImage) {
      this.imgA = document.createElement('img');
      this.imgA.className = 'pet-img is-front';
      this.imgA.title = this.pet.name;
      this.imgA.draggable = false;
      const idleName = (this.animations.idle && this.animations.idle[0]);
      if (idleName) this.imgA.src = this.assetBase + encodeURIComponent(idleName) + '.png';
      this.imgB = document.createElement('img');
      this.imgB.className = 'pet-img';
      this.imgB.title = this.pet.name;
  ...
  dispose() {
    ...
    if (this.clickRestoreTimer !== null) window.clearTimeout(this.clickRestoreTimer);
    if (this.chatClose) {
      this.chatClose();
      this.chatClose = null;
    }
  ...
  switchImage(next) {
    if (!next) return;
    const base = this.assetBase + encodeURIComponent(next) + '.png';
    const target = this.imgFront === 0 ? this.imgB : this.imgA;
    const el = target;
    if (!el) return;
    ...
    const onReady = () => {
      ...
      const old = this.imgFront === 0 ? this.imgA : this.imgB;
      el.classList.add('is-front');
      if (old && old !== el) old.classList.remove('is-front');
      this.imgFront = this.imgFront === 0 ? 1 : 0;
      el.style.transform = this.facing === 'right' ? 'scaleX(-1)' : '';
      if (this.pendingSquash) {
        this.pendingSquash = false;
        this.startSquash(el);
  ...
  startSquash(el, depth = S.SQ_SQUASH) {
    if (!el) return; // 静态图宠物无 video 元素：跳过挤压
  ...
  onClick() {
    ...
    // 静态图宠物无 ended：点击姿态短暂展示（约 400ms）后恢复 idle
    if (this.isImage && this.animations.idle.length) {
      if (this.clickRestoreTimer) window.clearTimeout(this.clickRestoreTimer);
      this.clickRestoreTimer = window.setTimeout(() => {
        this.pendingSquash = false;
        this.playOnce(S.pick(this.animations.idle, this.anim));
      }, 400);
    }
  }
  ...
  showWhisper(text) {
    ...
    this.whisperTimer = window.setTimeout(() => {
      this.whisperOn = false;
      this.renderBubble();
      if (this.isImage && this.animations.idle.length) this.playOnce(S.pick(this.animations.idle, this.anim));
    }, BUBBLE_DURATION_MS);
    this.playOnce(name);
  }
