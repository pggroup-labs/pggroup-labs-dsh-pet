...
function petsFromEnv() {
  try {
    ...
    if (Array.isArray(arr) && arr.length > 0) {
      return arr.map((p, i) => {
        const size = Number(p?.size) > 0 ? Number(p.size) : 462;
        return {
          ...
          media: p?.media === 'image' ? 'image' : undefined,
          index: i,
        };
      });
    }
...
function petWindowSize(pet) {
  const aspect = typeof pet?.aspect === 'number' && pet.aspect > 0 ? pet.aspect : 9 / 16;
  const height = pet.size * aspect;
  const bottomPad = (pet.size * aspect * (360 - 330)) / 360;
  const m = Math.round(pet.size * 0.5);
  return { width: Math.round(pet.size) + m * 2, height: Math.round(height + bottomPad) + m * 2 };
}
...
function createPetWindows() {
  ...
  for (const pet of pets) {
    const { width, height } = petWindowSize(pet);
